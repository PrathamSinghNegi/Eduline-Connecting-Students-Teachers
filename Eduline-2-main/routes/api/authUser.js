...
router.delete("/:appointment_id", authUser, async (req, res) => {
  try {
    ...
    const educatorId = user.appointments.map((item) =>
      item.id === req.params.appointment_id ? item.educator : null
    );

    const newId = educatorId.filter((educator) => educator !== null);

    const profile = await Profile.findOne({ educator: newId[0] });
    ...
    if (profile !== null) {
      const indexEducator = profile.students
        .map((item) => item.bookingId)
        .indexOf(removeIndex);

      profile.students.splice(indexEducator, 1);
