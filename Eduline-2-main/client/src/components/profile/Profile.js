...
const Profile = ({
    getProfileById,
    profile: {profileById, loading}, authEducator,authUser, match
}) => {
    useEffect(() => {
                                ...
                                <div class="current-profile">
                                    <div class="current-btn">
                                        ...
                                        {authUser.isUserAuthenticated ? (
                                            <Fragment>
                                                <Link to={`/appointment/${profileById.educator._id}`} type="button" className="rounded-pill btn btn-info"><i className="fas fa-calendar-check"></i> Book Appointment</Link>
                                            </Fragment>
                                            ) : (
                                                <Fragment>
                                        ...
                                        {authEducator.isEducatorAuthenticated && 
                                            authEducator.loadingEducator === false && 
                                            authEducator.educator._id === profileById.educator._id && (
                                    ...
                                    <div class="student-review">
                                            <h2 class="exp-common-heading">User Reviews</h2> 
                                            { authUser.isUserAuthenticated ? 
                                                (
                                                   <ReviewForm educatorId={profileById.educator._id} /> 
                                                ) : ""
                                            }
                                            {
                                                profileById.review.map(rev => (
                                                    <ProfileReview key={rev._id} review={rev} educatorId={match.params.id} />
                                                ))
                                            }
                                        </div>
                                </div>
